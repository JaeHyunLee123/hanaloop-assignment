...
* **제품 단위당 PCF 계산 수식**: 단순 총 배출량 / 총 생산량이 아닌, **1대를 만들 때 들어가는 '표준 부품 비율(BOM)'을 기준으로 고정된 수식**을 적용하여 제품 단위 PCF를 산출함 (재고 변동으로 인한 단위당 배출량 왜곡 방지 목적).
...
2. `ExtendedGhgEmission` 객체를 생성하여 대상 `Company.emissions`에 누적(`push`).
   * *조건*: 해당 회사, 월(`yearMonth`), 배출원(`source`), 배출 범위(`scope`), 생애주기(`pcfStage`)가 모두 완전히 일치할 경우에만 기존 객체에 합산하여 데이터 충돌을 방지함.
3. `Post` 객체로 입력 이력 기록.
   * *조건*: 사용자의 한 번의 입력으로 배출량이 할당된 **각 회사(Company)별로 각각 `Post` 객체가 생성**됨. (과제 요구사항에 따라 `Post` 객체의 프로퍼티 키 및 예시 값(`title`, `dateTime: "2026-05"` 등)은 제공된 형태를 엄격히 유지함). 동월 이력이 존재하면 `content`에 줄바꿈으로 내용을 덧붙임.
4. **데이터 수정 및 취소 정책 (Rollback)**: MVP 스코프 내에서는 이미 입력된 데이터에 대한 자체적인 수정/삭제 파이프라인을 두지 않으며, 오입력 시 마이너스(-) 수치를 추가로 입력하여 차감하는 방식으로 대응함.
